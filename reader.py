...
class ScheduleReader:
    ...
    def get_shifts_by_position(self, position):
        ...
        for index, row in self.df.iterrows():
            if position == row[0]:
                inPosition = True
            elif row[0] == 'CART ATTENDANTS' or row[0] == 'BEV CART ATTENDANTS' or row[0] == 'STARTERS/MARSHALS' or row[0] == 'GOLF SHOP':
                inPosition = False
            elif inPosition:
                print('Name:', row[0])
                print('Phone Number:', row[1])
